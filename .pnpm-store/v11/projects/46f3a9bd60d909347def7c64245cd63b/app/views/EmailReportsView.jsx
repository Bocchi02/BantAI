...
function EmailReportsPage() {
    ...
    const submit = async (event) => {
        ...
        if (!body.trim() || body.length > 10000 || !sender.trim() || !confirmed) {
            setError("Enter a sender and email body, then confirm consent before submitting.");
            return;
        }
        setSubmitting(true);
        setError("");
        ...
        <section className="lg:col-span-5 sneat-card p-5 sm:p-6" aria-labelledby="email-report-form-title">
          ...
          <form className="space-y-4" onSubmit={submit}>
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-[#384551]">Sender</span>
                <input required value={sender} onChange={(event) => setSender(event.target.value)} placeholder="sender@example.com" maxLength={320} className="h-10 px-3 rounded-md border border-[#d9dee3] text-xs bg-white text-[#384551] focus:ring-2 focus:ring-[#696cff]/20 focus:border-[#696cff] outline-none"/>
              </label>
            </div>
            <label className="flex flex-col gap-1">
            ...
            <label className="flex flex-col gap-1">
              ...
              <textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Paste the email message text (maximum 10,000 characters)..." rows={5} maxLength={10000} required className="p-3 rounded-md border border-[#d9dee3] text-xs bg-white text-[#384551] focus:ring-2 focus:ring-[#696cff]/20 focus:border-[#696cff] outline-none resize-y"/>
            ...
            <label className="flex gap-3 items-start text-sm text-[#646e78]"><input className="mt-1 w-4 h-4 shrink-0" type="checkbox" required checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)}/><span>I consent to sending this email content for encrypted storage and administrator training review. This is separate from routine detection and optional automatic contribution.</span></label>
            <button type="submit" disabled={submitting || !body.trim() || !confirmed} className="w-full mt-2 py-2.5 px-4 rounded-md bg-[#696cff] hover:bg-[#5f61e6] text-white text-xs font-bold shadow-[0_2px_4px_0_rgba(105,108,255,0.4)] transition disabled:opacity-50">
