...
function LandingPage({ authenticated, registrationEnabled = false, onNavigate, }) {
    const publicPrimaryPath = registrationEnabled ? "/register" : "/login";
      ...
      <header className="w-full bg-white/90 backdrop-blur-md border-b border-[#e4e6e8] sticky top-0 z-30 shadow-[0_2px_6px_0_rgba(67,89,113,0.06)]">
        <div className="max-w-[1440px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 h-18 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {(authenticated || registrationEnabled) && (<button className="text-xs sm:text-sm font-semibold bg-[#696cff] hover:bg-[#5f61e6] text-white px-4 sm:px-5 py-2.5 rounded-md transition-all shadow-[0_2px_4px_0_rgba(105,108,255,0.4)] active:scale-95" onClick={() => onNavigate(authenticated ? "/dashboard" : publicPrimaryPath)}>
      ...
      <section className="w-full max-w-[1440px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-16 pb-20 grid lg:grid-cols-12 gap-12 items-center" aria-labelledby="landing-title">
        <div className="lg:col-span-7 flex flex-col">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#e7e7ff] border border-[#c3c4ff] text-[#696cff] text-xs font-semibold tracking-wider uppercase mb-6 w-fit shadow-2xs">
            ...
            SERVER DETECTION · PRIVACY-MINIMIZED INSIGHTS
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="sneat-card p-5 flex gap-4 items-start">
            ...
            <div>
              ...
              <p className="text-xs text-[#8592a3] mt-1 leading-relaxed">Opened supported emails receive automatic redacted review; URL review runs only after the server URL model warns and sends the origin only.</p>
            </div>
          </div>
          <div className="sneat-card p-5 flex gap-4 items-start">
      ...
      <section className="w-full max-w-[1440px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-20" id="how-it-works" aria-labelledby="how-title">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <article className="sneat-card p-6 sm:p-7 flex flex-col gap-3">
            ...
            <p className="text-xs sm:text-sm text-[#646e78] leading-relaxed">Redacted cloud review adds context while deterministic server rules remain responsible for the final result.</p>
          </article>
          <article className="sneat-card p-6 sm:p-7 flex flex-col gap-3">
        ...
        <div className="p-8 sm:p-12 rounded-xl bg-gradient-to-br from-[#071e4a] to-[#04142f] text-white shadow-[0_4px_18px_0_rgba(34,48,62,0.18)] flex flex-col lg:flex-row items-center gap-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-2xl">
              Exact addresses and supported opened-email content travel over HTTPS for transient checking, but remain outside routine activity history. Gemini receives bounded redacted email context or a minimized website origin only.
            </p>
          </div>
          <ul className="grid sm:grid-cols-2 gap-3 text-xs text-slate-300 shrink-0">
            ...
            <li className="flex items-center gap-2 sm:col-span-2"><CheckIcon className="w-4 h-4 text-[#71dd37] shrink-0"/> Full-URL reports and optional 10% contributions use separate, explicit consent workflows</li>
          </ul>
        </div>
      </section>
