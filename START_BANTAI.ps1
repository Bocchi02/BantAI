...
if ($null -eq $Docker) {
    throw "Docker Desktop is required to start Signalam. Install or start Docker Desktop, then try again."
}
...
if (Test-Path -LiteralPath $Python -PathType Leaf) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Signalam model verification failed."
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Signalam Docker services could not be started."
}

Write-Host "Signalam services are starting in Docker." -ForegroundColor Green
