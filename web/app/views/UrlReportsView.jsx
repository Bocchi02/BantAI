...
function UrlReportsPage() {
    ...
    return (<>
      ...
      <div className="p-4 rounded-lg bg-[#e7e7ff]/50 border border-[#c3c4ff] flex items-center gap-3 text-xs text-[#4347d9] mb-6">
        ...
        <div>
          <strong className="block font-bold">Only addresses you submit are collected in full</strong>
          <p className="text-[#646e78] mt-0.5">Routine website history stores origins only. When you report a website result, the complete website address, including its path, is sent for review.</p>
        </div>
      </div>
                ...
                <tbody className="divide-y divide-[#e4e6e8]/70">
                  {data.items.map((report) => (<tr key={report.id} className="hover:bg-[#fbfbfd]">
                      <td className="py-3 px-3.5 max-w-[200px]">
                        ...
                        <small className="text-[#8592a3] block text-[11px]">{feedbackReasonLabel(report.reason)}</small>
                      </td>
