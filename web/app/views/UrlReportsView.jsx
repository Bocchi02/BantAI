...
function UrlReportsPage() {
    ...
    return (<>
      ...
      <div className="grid lg:grid-cols-12 gap-6 mb-6">
        <section className="lg:col-span-8 p-5 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm" aria-labelledby="new-url-report-title">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
            <div>
              <p className="text-xs font-semibold text-[#087EFF] uppercase tracking-wider">NEW REPORT</p>
          ...
          <form className="space-y-4" onSubmit={submit}>
            <label className="flex flex-col gap-1">
              ...
              <input type="url" inputMode="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com" maxLength={2048} autoCapitalize="none" autoComplete="off" spellCheck={false} aria-describedby="url-report-help" required className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm focus:ring-2 focus:ring-[#087EFF] outline-none"/>
            </label>
            <p id="url-report-help" className="text-xs text-slate-400">Paste the complete address from your browser. BantAI retains its path for administrator review and future training-data assessment.</p>
            <label className="flex flex-col gap-1">
            ...
            <fieldset className="space-y-1 border-0 p-0 m-0">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <label className={cx("p-3.5 rounded-xl border cursor-pointer flex gap-3 items-start transition", classification === "LEGITIMATE" ? "bg-emerald-50 border-emerald-300 text-emerald-950 ring-2 ring-emerald-200" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50")}>
                  ...
                  <div>
                    <strong className="text-xs sm:text-sm font-bold block">Legitimate website</strong>
                    <small className="text-xs text-slate-500 leading-snug">The detection may have been too cautious.</small>
                  </div>
                </label>
                <label className={cx("p-3.5 rounded-xl border cursor-pointer flex gap-3 items-start transition", classification === "SUSPICIOUS" ? "bg-rose-50 border-rose-300 text-rose-950 ring-2 ring-rose-200" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50")}>
                  ...
                  <div>
                    <strong className="text-xs sm:text-sm font-bold block">Suspicious website</strong>
                    <small className="text-xs text-slate-500 leading-snug">The detection may have missed warning signs.</small>
                  </div>
                </label>
              </div>
            </fieldset>
            ...
            <button className="h-10 px-5 rounded-xl bg-[#087EFF] hover:bg-[#071E4A] text-white text-xs font-bold shadow-sm transition disabled:opacity-50" disabled={busy || !url.trim() || !detectorOutcome}>
              {busy ? "Submitting..." : "Submit for review"}
            </button>
          </form>
        </section>
        <aside className="lg:col-span-4 p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-[#071E4A] to-[#04142F] text-white flex flex-col justify-between" aria-label="Report privacy information">
          <div>
            ...
            <p className="text-xs font-semibold text-[#1495FF] uppercase tracking-wider mb-1">EXPLICIT REPORT</p>
            ...
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                <tr>
                  ...
                  <th className="pb-3 px-3.5">Your feedback</th>
                  <th className="pb-3 px-3.5">Training review</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.items.map((report) => (<tr key={report.id} className="hover:bg-slate-50">
                    <td className="py-3 px-3.5 max-w-xs">
                      ...
                      <small className="text-xs text-slate-400 block">{feedbackReasonLabel(report.feedback_reason)}</small>
                    </td>
                    <td className="py-3 px-3.5"><StatusBadge outcome={report.detector_outcome}/></td>
                    <td className="py-3 px-3.5">
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-[#071E4A]">{feedbackVerdictLabel(report.feedback_verdict)}</span>
                      <small className="text-xs text-slate-400 block mt-0.5">{userClassificationLabel(report.user_classification)}</small>
                    </td>
                    <td className="py-3 px-3.5">
                      ...
                      {report.reviewed_at && <small className="text-xs text-slate-400 block mt-0.5">Reviewed {niceDate(report.reviewed_at)}</small>}
                    </td>
      ...
      <p className="text-xs text-slate-400 text-center max-w-2xl mx-auto mt-6 leading-relaxed"><strong>Reports are decision-support feedback.</strong> They do not automatically retrain the frozen detector or guarantee that a website is legitimate or malicious.</p>
    </>);
}
