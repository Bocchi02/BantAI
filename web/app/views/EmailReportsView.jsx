...
function EmailReportsPage() {
    ...
    return (<>
      ...
      <div className="grid lg:grid-cols-12 gap-6 mb-6">
        <section className="lg:col-span-8 p-5 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm" aria-labelledby="new-email-report-title">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
            <div>
              <p className="text-xs font-semibold text-[#087EFF] uppercase tracking-wider">NEW EMAIL REPORT</p>
              <h2 id="new-email-report-title" className="text-base font-bold text-[#04142F]">Enter the email details</h2>
            </div>
            <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">Explicit submission</span>
          </div>
          <form className="space-y-4" onSubmit={submit}>
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-slate-700">Email provider</span>
                <select value={provider} onChange={(event) => setProvider(event.target.value)} className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#087EFF] outline-none">
              ...
              <label className="flex flex-col gap-1">
                ...
                <input value={sender} onChange={(event) => setSender(event.target.value)} maxLength={320} placeholder="Sender name or address" required className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm focus:ring-2 focus:ring-[#087EFF] outline-none"/>
              </label>
            </div>
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-slate-700">Subject</span>
              <input value={subject} onChange={(event) => setSubject(event.target.value)} maxLength={500} placeholder="Email subject, if available" className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm focus:ring-2 focus:ring-[#087EFF] outline-none"/>
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-slate-700">Email body</span>
              <textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={10000} rows={6} placeholder="Paste the email message here" aria-describedby="email-body-help" required className="w-full p-3 rounded-xl border border-slate-300 text-xs sm:text-sm font-mono focus:ring-2 focus:ring-[#087EFF] outline-none"/>
            </label>
            <p id="email-body-help" className="text-xs text-slate-400">The body is encrypted before database storage. It cannot be read from the user or administrator interface and is reserved for an approved offline training process.</p>
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-slate-700">What result did BantAI show?</span>
              <select value={detectorOutcome} onChange={(event) => setDetectorOutcome(event.target.value)} required className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#087EFF] outline-none">
            ...
            <fieldset className="space-y-1 border-0 p-0 m-0">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <label className={cx("p-3.5 rounded-xl border cursor-pointer flex gap-3 items-start transition", classification === "LEGITIMATE" ? "bg-emerald-50 border-emerald-300 text-emerald-950 ring-2 ring-emerald-200" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50")}>
                  ...
                  <div>
                    <strong className="text-xs sm:text-sm font-bold block">Seems legitimate</strong>
                ...
                <label className={cx("p-3.5 rounded-xl border cursor-pointer flex gap-3 items-start transition", classification === "SUSPICIOUS" ? "bg-rose-50 border-rose-300 text-rose-950 ring-2 ring-rose-200" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50")}>
                  ...
                  <div>
                    <strong className="text-xs sm:text-sm font-bold block">Seems suspicious</strong>
            ...
            <label className="flex flex-col gap-1">
              ...
              <select value={reason} onChange={(event) => setReason(event.target.value)} className="h-10 px-3 rounded-xl border border-slate-300 text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#087EFF] outline-none">
            ...
            <label className="flex items-start gap-3 p-3.5 rounded-xl bg-blue-50/60 border border-blue-200 cursor-pointer">
              ...
              <div className="text-xs">
                <strong className="text-[#071E4A] block font-semibold">Include this encrypted email in the reporting workflow.</strong>
            ...
            <button className="h-10 px-5 rounded-xl bg-[#087EFF] hover:bg-[#071E4A] text-white text-xs font-bold shadow-sm transition disabled:opacity-50" disabled={busy || !sender.trim() || !body.trim() || !detectorOutcome || !confirmed}>
              {busy ? "Encrypting and submitting..." : "Submit encrypted report"}
            </button>
          </form>
        </section>
        <aside className="lg:col-span-4 p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-[#071E4A] to-[#04142F] text-white flex flex-col justify-between" aria-label="Email training privacy information">
          <div>
            ...
            <p className="text-xs font-semibold text-[#1495FF] uppercase tracking-wider mb-1">ENCRYPTED CONTENT</p>
      ...
      <section className="p-5 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
          <div>
            <p className="text-xs font-semibold text-[#087EFF] uppercase tracking-wider">YOUR SUBMISSIONS</p>
            <h2 className="text-base font-bold text-[#04142F]">{data ? `${data.total} email ${data.total === 1 ? "report" : "reports"}` : "Loading reports..."}</h2>
          </div>
          <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">Body encrypted</span>
        </div>
        {data && data.items.length > 0 ? (<div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                <tr>
                  <th className="pb-3 px-3.5">Email</th>
                  <th className="pb-3 px-3.5">Detector result</th>
                  <th className="pb-3 px-3.5">Your label</th>
                  <th className="pb-3 px-3.5">Training assessment</th>
                  <th className="pb-3 px-3.5">Submitted</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {data.items.map((report) => (<tr key={report.id} className="hover:bg-slate-50">
                    <td className="py-3 px-3.5 max-w-xs">
                      ...
                      <small className="text-xs text-slate-400 block">{report.sender} · {report.provider.toUpperCase()} · Encrypted body included</small>
                    </td>
                    <td className="py-3 px-3.5"><StatusBadge outcome={report.detector_outcome}/></td>
                    <td className="py-3 px-3.5">
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-[#071E4A]">{userClassificationLabel(report.user_classification)}</span>
                    </td>
                    <td className="py-3 px-3.5">
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">{trainingStatusLabel(report.training_status)}</span>
                    </td>
                    <td className="py-3 px-3.5 text-xs text-slate-400 whitespace-nowrap">{niceDate(report.submitted_at)}</td>
                  </tr>))}
              </tbody>
            </table>
      ...
      <p className="text-xs text-slate-400 text-center max-w-2xl mx-auto mt-6 leading-relaxed"><strong>Training reference only.</strong> Submission does not retrain XLM-R V1 or guarantee that the email is legitimate or malicious.</p>
    </>);
}
