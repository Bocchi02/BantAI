...
import { api } from "../api";
import { CheckCircle2Icon, HelpCircleIcon, RefreshCwIcon, SearchIcon } from "../Icons";
...
function ActivityPage() {
    ...
    const [filters, setFilters] = useState({ search: "", event_type: "", outcome: "", provider: "", date_from: "", date_to: "" });
    ...
    const openDetails = useCallback(async (item, force = false) => {
        ...
        try {
            if (!item.detail_reference)
                throw new Error("This older activity record has no temporary explanation reference. Complete the check again to create one.");
    ...
    return (<>
      ...
      <section className="sneat-card p-4 sm:p-5 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 text-xs">
          <label className="flex flex-col gap-1 sm:col-span-2 lg:col-span-1">
            <span className="font-semibold text-[#384551] text-xs">Search retained metadata</span>
            <span className="relative">
              <SearchIcon className="w-4 h-4 text-[#8592a3] absolute left-3 top-1/2 -translate-y-1/2" aria-hidden="true"/>
              <input type="search" value={filters.search} onChange={(event) => change("search", event.target.value)} placeholder="Origin, sender, or subject" maxLength={320} className="w-full h-9 pl-9 pr-3 rounded-md border border-[#d9dee3] bg-white text-xs text-[#384551] focus:ring-2 focus:ring-[#696cff]/20 focus:border-[#696cff] outline-none"/>
            </span>
          </label>
          <label className="flex flex-col gap-1">
      ...
      <section className="sneat-card p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-[#e4e6e8]/70">
          <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => void load()} disabled={refreshing} className="h-9 px-3 rounded-md border border-[#d9dee3] text-xs font-semibold text-[#646e78] hover:bg-[#f5f5f9] flex items-center gap-1.5">
              <RefreshCwIcon className={cx("w-3.5 h-3.5", refreshing && "animate-spin")} aria-hidden="true"/>
              {refreshing ? "Refreshing…" : "Refresh"}
            </button>
          </div>
        </div>
        {data ? <ActivityTable items={data.items} onFeedback={setFeedbackActivity} onDetails={openDetails}/> : <DashboardSkeleton />}
