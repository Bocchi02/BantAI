...
function HelpView({ onNavigate, registrationEnabled = false }) {
    return (<>
      ...
      <div className="p-4 rounded-lg bg-[#fff1d6] border border-[#ffdd99] flex items-start gap-3 text-sm text-[#664400] mb-6" role="note">
        ...
        <p><strong>Signing in alone does not activate detection.</strong> The Signalam server models must be ready, the extension must be installed, and that browser extension must be connected to your account.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <HelpCard icon={ShieldCheckIcon} title={registrationEnabled ? "1. Create an account" : "1. Use your provisioned account"}>
          ...
          <p>End users do not install Python, Docker, model files, a local API, or Signalam Companion.</p>
        </HelpCard>
        ...
        <HelpCard icon={GlobeIcon} title="2. Install the browser extension">
          <ol className="list-decimal pl-5 space-y-2">
            ...
            <li>Enable <strong>Developer mode</strong>, choose <strong>Load unpacked</strong>, and select the existing <code className="px-1.5 py-0.5 rounded bg-[#f5f5f9] text-[#384551]">extension</code> folder inside this Signalam project.</li>
            <li>Pin Signalam if desired, then refresh any already-open Gmail, Outlook, or Yahoo Mail tab.</li>
          </ol>
          <p>Signalam scans the exact active address-bar URL. It does not crawl pages, follow redirects, inspect TLS, or scan embedded links.</p>
        </HelpCard>
        ...
        <HelpCard icon={LaptopIcon} title="3. Connect or revoke the extension">
          <ol className="list-decimal pl-5 space-y-2">
            ...
            <li>Open the Signalam extension popup and enter the eight-character code before it expires.</li>
      ...
      <section className="sneat-card p-5 sm:p-6 mt-6" aria-labelledby="troubleshooting-title">
        ...
        <dl className="grid md:grid-cols-2 gap-4 text-sm">
          <div className="p-4 rounded-lg bg-[#f5f5f9]"><dt className="font-bold text-[#384551]">Server models unavailable</dt><dd className="mt-1">Wait briefly and retry. Signalam shows Service unavailable and does not substitute another model or present a safe result.</dd></div>
          <div className="p-4 rounded-lg bg-[#f5f5f9]"><dt className="font-bold text-[#384551]">Cloud review unavailable</dt><dd className="mt-1">The frozen server checks may still complete, but Signalam reports the cloud layer as unavailable instead of inventing a result.</dd></div>
          <div className="p-4 rounded-lg bg-[#f5f5f9]"><dt className="font-bold text-[#384551]">Extension connection failed</dt><dd className="mt-1">Confirm the account is active, generate a fresh one-time code, and check that the configured Signalam HTTPS service is reachable.</dd></div>
