...
const OUTCOMES = [
    { value: "NO_STRONG_WARNING_SIGNS", label: "No strong warning signs.", short: "No strong warning signs." },
...
function outcomeInfo(outcome) {
    return OUTCOMES.find((item) => item.value === outcome) || { label: "Result unavailable", short: "Unavailable" };
}
      ...
      <table className="w-full text-left text-xs text-[#646e78]">
        <thead className="text-[11px] font-bold text-[#8592a3] uppercase tracking-wider bg-[#f5f5f9]/80 border-b border-[#e4e6e8]">
          <tr>
            ...
            {onDetails && <th className="py-3 px-3.5"><span className="sr-only">Actions</span></th>}
          </tr>
        </thead>
        <tbody className="divide-y divide-[#e4e6e8]/70">
          {items.map((item) => (<tr key={item.id} className="hover:bg-[#fbfbfd] transition-colors">
              ...
              <td className="py-3.5 px-3.5">
                ...
                <small className="text-[#8592a3] block mt-1 text-xs">
                  {item.cloud_status === "COMPLETE" ? "Cloud review complete" : item.cloud_status === "SKIPPED" ? "Cloud review not required" : "Cloud review unavailable"}
                </small>
              </td>
              <td className="py-3.5 px-3.5 whitespace-nowrap text-[#8592a3] text-[11px]">
                ...
                {Number.isFinite(item.duration_ms) && <small className="block text-xs mt-1">{item.duration_ms.toLocaleString()} ms</small>}
              </td>
              {onFeedback && (<td className="py-3.5 px-3.5">
                  {item.feedback_submitted ? (<div><span className="text-xs font-bold text-[#2d5816] bg-[#e8fadf] px-2 py-0.5 rounded-full border border-[#c6f1af]">{trainingStatusLabel(item.feedback_status)}</span>{item.feedback_assessment && <small className="block mt-1 text-xs text-[#646e78]">Admin label: {item.feedback_assessment === "LEGITIMATE" ? "Legitimate" : "Suspicious"}</small>}</div>) : item.event_type === "URL" ? (<button className="text-xs font-semibold text-[#696cff] hover:underline" type="button" onClick={() => onFeedback(item)}>
                        Give feedback
                      </button>) : (<span className="text-[#8592a3]">Not reported</span>)}
                </td>)}
              {onDetails && (<td className="py-3.5 px-3.5 text-right">
                  <button className="text-xs font-semibold text-[#696cff] hover:underline whitespace-nowrap" type="button" onClick={() => onDetails(item)}>
                    View explanation
                  </button>
                </td>)}
            </tr>))}
